...
public sealed partial class ClusterRendererSystem
{
    ...
    private const int LightStride = GpuLight.SizeInBytes;
        ...
        RenderLightSet lights)
    {
        ...
        uint rasterBinCount = snapshot.RasterBinCount;
        uint deformBinCount = snapshot.DeformBinCount;
        uint shadeBinCount = snapshot.ShadeBinCount;
        ...
        frame.RasterBinMeta = frame.AddBuffer(
            ref graph,
            checked((ulong)rasterBinCount * RasterBinStride),
            BufferUsages.None,
            "Cluster raster bins");
        ...
        frame.BinnedDrawArgs = frame.AddBuffer(
            ref graph,
            checked((ulong)rasterBinCount * ClusterIndirectAbi.DrawBytes),
            BufferUsages.None,
        ...
        frame.SoftwareDispatchArgs = frame.AddBuffer(
            ref graph,
            checked((ulong)rasterBinCount * 2 * ClusterIndirectAbi.DispatchBytes),
            BufferUsages.Indirect,
        ...
        frame.DeformBinMeta = frame.AddBuffer(
            ref graph,
            checked((ulong)deformBinCount * DeformBinStride),
            BufferUsages.None,
            "Cluster deform bins");
        ...
        frame.DeformDispatchArgs = frame.AddBuffer(
            ref graph,
            checked((ulong)deformBinCount * ClusterIndirectAbi.DispatchBytes),
            BufferUsages.Indirect,
        ...
        frame.ShadeBinCounts = frame.AddBuffer(
            ref graph,
            checked((ulong)shadeBinCount * sizeof(uint)),
            BufferUsages.None,
        ...
        frame.ShadeIndirectArgs = frame.AddBuffer(
            ref graph,
            checked((ulong)shadeBinCount * ClusterIndirectAbi.DispatchBytes),
            BufferUsages.Indirect,
        ...
        RenderLightSet source)
    {
        ...
        GpuLightUpload upload = (_gpuLightBuffers
            ?? throw new InvalidOperationException("The render-light GPU adapter was not created."))
            .Upload(generation, source);

        EnsureLightStructureResources(
            frame.Width,
            frame.Height,
            checked((int)upload.DirectionalCount),
            checked((int)upload.PointCount),
            checked((int)upload.SpotCount));
    ...
    private struct FrameResources
    {
        ...
        internal int Width;
        internal int Height;
        internal uint TargetLayer;
        internal uint MaterialCount;
        internal uint RasterBinCount;
        internal uint DeformBinCount;
        internal uint ShadeBinCount;
